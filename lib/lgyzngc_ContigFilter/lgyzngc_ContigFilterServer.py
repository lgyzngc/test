...
def get_config():
    ...
    for nameval in config.items(get_service_name() or 'lgyzngc_ContigFilter'):
        retconfig[nameval[0]] = nameval[1]
    return retconfig

config = get_config()

from lgyzngc_ContigFilter.lgyzngc_ContigFilterImpl import lgyzngc_ContigFilter
impl_lgyzngc_ContigFilter = lgyzngc_ContigFilter(config)
...
class Application(object):
    ...
    def __init__(self):
        submod = get_service_name() or 'lgyzngc_ContigFilter'
        ...
        self.rpc_service.add(impl_lgyzngc_ContigFilter.filter_contigs,
                             name='lgyzngc_ContigFilter.filter_contigs',
                             types=[dict])
        self.method_authentication['lgyzngc_ContigFilter.filter_contigs'] = 'required'
        self.rpc_service.add(impl_lgyzngc_ContigFilter.status,
                             name='lgyzngc_ContigFilter.status',
                             types=[dict])
                    ...
                    if auth_req != "none":
                        if token is None and auth_req == 'required':
                            ...
                            err.data = "Authentication required for " + \
                                "lgyzngc_ContigFilter but no authentication header was passed"
                            raise err
